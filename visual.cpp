...
const IMGHDR imgPointer = {5,5,0x1,(unsigned char *)Pointer};
...
void EditCoordinates(void *rect_or_xy, int is_rect)
{
    ...
    if (!is_rect)
    {
        unsigned int *xy=( unsigned int *)rect_or_xy;
        rect_gui->x_pos=xy[0];
        rect_gui->y_pos=xy[1];
        rect_gui->xy=xy;
    }
    ...
    rect_gui->gui.canvas=(RECT *)(&Canvas_1);
...
{
    FVIEW *fview=(FVIEW*)MenuGetUserPointer(data);
    FLIST *fl;
    int i, n;
